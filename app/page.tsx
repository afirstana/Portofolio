...
export default function HomePage() {
  ...
  return <main className="site-shell"><StructuredData /><ScrollProgress /><SiteHeader /><HeroCinematic hero={hero} />
    <Reveal direction="up" stagger={true}>
      <section id="about" className="section page-width about-layout">
        <p className="section-label mono">{about.eyebrow}</p>
        <div className="about-content">
          <div className="system-portrait" role="img" aria-label={about.photo}>
            <span />
            <i />
            <b />
          </div>
          <div>
            ...
            <p className="body-copy">{about.bio_text}</p>
    ...
    <Reveal direction="up" threshold={0.06}>
      <ProjectExplorer projects={projects} />
    </Reveal>

    <Reveal direction="up" stagger={true} threshold={0.08}>
      <section id="method" className="section method-section">
        <div className="page-width">
          <p className="section-label mono">{method.eyebrow}</p>
          <h2 className="section-title narrow-title">{method.heading}</h2>
    ...
    <Reveal direction="up" threshold={0.08}>
      <SkillMatrix content={skills} projects={projects} />
    </Reveal>

    <Reveal direction="up" threshold={0.06}>
      <DataPlayground data={playgroundData} />
    </Reveal>

    <Reveal direction="up" stagger={true} threshold={0.08}>
      <TimelineExplorer content={timeline} />
    </Reveal>

    <Reveal direction="up" threshold={0.08}>
      <section id="contact" className="contact-section">
        <div className="page-width">
          <p className="section-label mono">{contact.eyebrow}</p>
          <h2>{contact.heading}</h2>
          <a className="contact-cta" href={`mailto:${contact.email}`}>
            {contact.cta_text} <span>↗</span>
          </a>
          <div className="contact-meta">
            <div>
              <a href={`mailto:${contact.email}`}>{contact.email}</a>
              <ContactActions email={contact.email} copyLabel={contact.copy_label} />
            </div>
            <div>
