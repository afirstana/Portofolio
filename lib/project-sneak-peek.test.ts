...
describe("Project Explorer sneak peek", () => {
  it("keeps content-backed preview metadata for every authored project", () => {
    ...
    expect(getProjects().find((project) => project.slug === "banking-fraud-3d-anomaly-manifold")?.preview.metrics).toContainEqual({ label: "Embedded Transactions", value: "2,512 Points" });
